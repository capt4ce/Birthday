...
if(!empty($images_arr)){ $count=0;
	foreach($images_arr as $image_src){
		$image_src=substr($image_src, 3, strlen($image_src));
		$count++;
		$sql="INSERT INTO picture_tbl(link) values('$image_src')";
		if (!mysqli_query($conn,$sql)) die("An error occur while uploading the pictures");
		?>
		<div class="col-md-2">
      <img src="<?php echo $image_src;?>" alt="" class="img-thumbnail pictures">
		</div>
<?php }
}
